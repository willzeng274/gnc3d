<script lang="ts">
	...
	function startGame(connectWs: boolean = false) {
		...
		if (connectWs) {
			...
			const url = PUBLIC_PROD === "true" ? "wss://gnc3d-backend.onrender.com/healthz" : "ws://localhost:8080";
			// const url = "192.168.0.84";
			const ws = new WebSocket(
				`${url}?username=${username}&room=${room}&sex=${
					sex === undefined ? "true" : sex ? "true" : "false"
				}&seed=${seed === 0 ? 1 : seed}`
			);
				...
				if (m.data instanceof ArrayBuffer) {
					...
					if (
						...
						if (arr[0] === 1) {
							...
							players[ind].animation = convertIntToAnimation(
								arr[11]
							);
						} else if (arr[0] === 3) {
							...
							if (arr[1] === 0) {
								...
								cakes = cakes.filter((c) => {
									if (c.id !== arr[2]) {
										...
										score.update((sc) =>
											getNewScoreByCakeType(sc, c.type)
										);
										return false;
									}
								});
	...
	<Suspense final on:load={() => (isSuspend = false)}>
        ...
        <li>Play</li> -->
					{#each contextMenuItems as ctx}
						<li>
							<button on:click={() => (currentCtx = ctx)}
								>{ctx.name}</button
							>
						</li>
					{/each}
		...
		{#if currentCtx.name === "Skins"}
			...
			<T.Group position.x={-3} position.z={3} in={zoomIn} out={zoomOut}>
				<T.Group
					rotation.y={rotation}
					position.y={$scale}
					scale={$scale}
					on:pointerenter={() =>
						$scale !== 2.5 && scale.set(sex === false ? 2 : 1.5)}
					on:pointerleave={() =>
						$scale !== 2.5 && scale.set(sex === false ? 2 : 1)}
					on:click={() => {
						if (sex === false) {
							scale.set(2.5);
			...
			<T.Group position.x={3} position.z={-3} in={zoomIn} out={zoomOut}>
				<T.Group
					...
					scale={$scale2}
					on:pointerenter={() =>
						$scale2 !== 2.5 && scale2.set(sex === true ? 2 : 1.5)}
					on:pointerleave={() =>
						$scale2 !== 2.5 && scale2.set(sex === true ? 2 : 1)}
					on:click={() => {
						if (sex) {
							scale2.set(2.5);
		...
		{:else if currentCtx.name === "Shop"}
			...
			<T.DirectionalLight
				position={[0, 10, 10]}
				castShadow
				intensity={isWizardUnlocked ? 1 : 0.1}
			/>
	...
	<Suspense final>
		...
		<Root>
			...
			{#if $socket === null}
				...
				>
			{:else}
				<button on:click={() => $socket?.close()} class="quitbtn"
					>Exit Game
				</button>
			{/if}
		</Root>
		...
		<CollisionGroups groups={[0]}>
			<Player
				{username}
				{host}
				sex={sex === undefined ? true : sex}
				bind:isWizardUnlocked
			/>
			<Door />
			{#if $socket !== null}
				{#if host}
